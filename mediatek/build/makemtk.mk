...
.PHONY : new newall remake remakeall clean cleanall clean-md32\
         ...
         pregen preclean \
         update-api modem-info bindergen clean-modem check-seandroid  zipbin
...
newall: cleanall pregen codegen custgen remakeall zipbin
cleanall: preclean
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
	$(hide) rm -f $(LOG)$@.log $(LOG)$@.log_err
	$(hide) ./makeMtk $(JZS_FULL_PROJECT) mm build/target/board/ snod $(DEAL_STDOUT_UPDATE_MD) && \
         $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
         $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
...
custgen: $(PRJ_MF)
	...
	$(hide) perl $(JZS_PATH_SOURCES)/build/tools/pregen.pl $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk $(PROJECT) $(FULL_PROJECT)
...
# ifneq ($(TARGET_PRODUCT),generic)
	$(hide) chmod a+x $(MKTOPDIR)/zlingyin.source/build/jztar.kk2.sh
	$(hide) cd $(MKTOPDIR)/zlingyin.source/build && \
		(./jztar.kk2.sh $(MKTOPDIR) && \
			cd $(MKTOPDIR))
# else
