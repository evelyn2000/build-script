...
.PHONY : new newall remake remakeall clean cleanall \
         ...
         pregen preclean \
         update-api modem-info bindergen clean-modem zipbin
...
ifeq ($(ENABLE_TEE), TRUE)
  ...
  DEAL_STDOUT_JZSJAVAOPTGEN := 2>&1 | tee -a $(LOG)javaoptgen.qs.log
  ...
  DEAL_STDOUT_JZSJAVAOPTGEN := > $(LOG)javaoptgen.qs.log 2>&1
...
newall: cleanall pregen codegen custgen update-api remakeall zipbin
cleanall: preclean
remakeall: pregen codegen custgen
new: clean remake
ifeq ($(MAKELEVEL),0)
clean: preclean
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
	$(hide) rm -f $(LOG)$@.log $(LOG)$@.log_err
	$(hide) ./makeMtk mm build/target/board/ snod $(DEAL_STDOUT_UPDATE_MD) && \
         $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
         $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
...
ifneq ($(QS_SUB_PROJECT_DWS_FILENAME), )
	$(hide) cp -f mediatek/custom/$(PROJECT)/kernel/dct/dct/$(QS_SUB_PROJECT_DWS_FILENAME).dws mediatek/custom/$(PROJECT)/kernel/dct/dct/codegen.dws
endif
...
JAVAJZSOPTFILE := zlingyin.source/frameworks/share/core/java/com/jzs/utils/ConfigOption.java
$(JAVAJZSOPTFILE): zlingyin.source/build/tools/javaoptgen.pl $(PRJ_MF) 
	$(hide) echo $(SHOWTIME) gen $@ ...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)javaoptgen.log
	$(hide) perl zlingyin.source/build/tools/javaoptgen.pl $(PRJ_MF)  $(DEAL_STDOUT_JZSJAVAOPTGEN)	

ALLJAVAOPTFILES := $(JAVAIMEOPTFILE) $(JAVAOPTFILE) $(JAVAJZSOPTFILE)
clean-javaoptgen:
ifneq ($(strip $(MTK_DEPENDENCY_AUTO_CHECK)), true)
	$(hide) echo $(SHOWTIME) $@ing ...
...
# ifneq ($(TARGET_PRODUCT),generic)
	$(hide) chmod a+x $(MKTOPDIR)/zlingyin.source/build/jztar.sh
	$(hide) cd $(MKTOPDIR)/zlingyin.source/build && \
		(./jztar.sh $(MKTOPDIR) && \
			cd $(MKTOPDIR))
# else
#	$(hide) echo no need to zip...
# endif	
