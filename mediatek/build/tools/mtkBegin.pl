...
if($ENV{"QS_PRJ"})
{
	$QsOutRootTempDir = $ENV{"JZS_OUT_COVER_DIR"};
	$QsOutRootTempDir = "out/qishangtempprjout" if($QsOutRootTempDir eq "");
	$QsOutRootDir = "$QsOutRootTempDir/out/target/product/$prj";
	#print "======jzs=========$QsOutRootTempDir=== \n";
	if(-e $QsOutRootDir){
		$srcDir = "$QsOutRootTempDir/out";
		$dstDir = "$out_dir";
		system("rsync -av --exclude=.svn --exclude=Thumbs.db --exclude=.git --exclude=.cvs $srcDir/ $dstDir/ > auto_sync_begin.log 2>&1");
	}
}
